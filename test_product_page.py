...
from .pages.locators import ProductPageLocators, BasePageLocators, MainPageLocators
...
class TestUserAddToBasketFromProductPage:
    @pytest.fixture(scope="function", autouse=True)
        def setup(self):
            main_page = MainPageLocators.MAIN_PAGE_URL
            main_page.open()
            main_page.go_to_login_page()
            f = faker.Faker()
            fake_email = f.email()
            password = passgen.passgen(12, False, True, True, "both")
            register_new_user
